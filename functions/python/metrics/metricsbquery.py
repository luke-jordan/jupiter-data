...
def count_users_in_list_that_performed_event(config):
...
def fetch_count_of_new_users_that_saved_between_period(start_time, end_time):
    ...
    return count_users_in_list_that_performed_event(
        {
            "event_type": USER_OPENED_APP_EVENT_CODE,
            "source_of_event": EXTERNAL_EVENT_SOURCE,
            "least_time_to_consider": start_time,
            "max_time_to_consider": end_time,
...
def calculate_percentage_of_users_who_performed_event_n_days_ago_and_have_not_performed_other_event(config):
    ...
    count_of_users_that_performed_start_event_n_days_ago = count_users_with_event_type(
        start_event, start_time_in_milliseconds_n_days_ago, end_time_in_milliseconds_n_days_ago, INTERNAL_EVENT_SOURCE
    )

    count_of_users_that_performed_start_event_n_days_ago_then_performed_other_event_afterwards = count_users_in_list_that_performed_event(
        {
            "event_type": next_event,
            "source_of_event": EXTERNAL_EVENT_SOURCE,
            "least_time_to_consider": convert_to_millisecond(
                calculate_date_n_days_ago(n_days_ago - 1), # day after n days ago
                HOUR_MARKING_START_OF_DAY
            ),
            "max_time_to_consider": convert_to_millisecond(
...
def fetch_avg_number_users_by_tx_type(config):
