...
def unzip_gzip(file, remove_original=True):
    ...
    with io.TextIOWrapper(gzip.open(filename, "r")) as f:
        lines = f.readlines()
        print("io wrapped text line")
        print(type(lines))
        print(lines)
    lines = [x.strip() for x in lines if x]

    print("stripped lines, creating new josn file")
    ...
    print("looping over lines that have been stripped")
    ...
    print("done creating json file, about to upload to json file to gcs imports folder")
...
def import_json_url(filename):
    return "gs://" + CLOUD_STORAGE_BUCKET + "/import_manual_python3/" + filename
...
# upload_file_to_gcs('amplitude.zip', YESTERDAY + '.zip', 'export_test')
...
for file in file_list('.gz'):
    ...
    upload_file_to_gcs("amplitude/" + file_json(file), file_json(file),
                           'import_manual_python3')
    upload_file_to_gcs("amplitude/" + "properties_" + file_json(file),
                       "properties_" + file_json(file), 'import_manual_python3')

    print("uploaded json file to gcs folder import_manual_python3")
    ...
    # Remove JSON file
    remove_file(file, "amplitude/{id}".format(id=ACCOUNT_ID))
